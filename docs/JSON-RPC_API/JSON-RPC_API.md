...
platone has pub/sub support. [Read this page](https://platone.ethereum.org/docs/rpc/server) for more information.
...
Then start the JSON-RPC proxy (defaults to '~/.ethereum/platone.ipc' and 'http://127.0.0.1:8545'): 
```bash
...
```bash
scripts/jsonrpcproxy.py <path to your node's platone.ipc> <URL for this proxy server>
```
...
```js
// Request
curl -X POST --data '{"jsonrpc":"2.0","method":"db_platoneex","params":["testDB","myKey"],"id":73}'
